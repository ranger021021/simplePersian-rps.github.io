...
ret.addEventListener('click',() => {
    ...
    fina.textContent = '';
    finaa.textContent = '';
    compgame.classList.add('remove');
    two.classList.add('remove');
    two.nextElementSibling.classList.add('remove');
    fina.classList.add('remove');
    finaa.classList.add('remove');
    clearRound();
});
...
finaa.style.backgroundColor= "yellowgreen";
    ...
    if(Number(scr1.textContent) >= winningScore){
        if(GameMode == 'comp'){
            ...
            finaa.textContent = ` تبریک! بازیکن شماره یک برنده این راند شد `;
        }
        ender =1;
        counter++;
        clearRound();
    ...
    if( Number(scr2.textContent) >= winningScore){
        if(GameMode == 'comp'){
            ...
            finaa.textContent = ` تبریک! بازیکن شماره دو برنده این راند شد `;
            finaa.style.backgroundColor= "red";
        }
        ender =1;
        counter++;
